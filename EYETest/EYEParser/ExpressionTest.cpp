...
namespace Eye
{
	namespace Parser
	{
		...
		TEST(ParserExpressionTest, NewExpression)
		{
			std::string testType = "Expression";
			std::string eyeFile = "NewExpression.eye";
			std::string testFile = "NewExpression.json";
			std::string eyeFilePath = "..\\..\\..\\..\\EYETest\\EYEParser\\" + testType + "\\" + eyeFile;
			std::string testFilePath = "..\\..\\..\\..\\EYETest\\EYEParser\\" + testType + "\\" + testFile;

			Lexer::Lexer lexer;
			if (!lexer.Tokenize(eyeFilePath))
				EYE_LOG_CRITICAL("EYETest->EYEParser->ParserExpressionTest->NewExpression Failed to Tokenize()");

			Parser parser;
			if (!parser.Parse(lexer.GetTokens()))
				EYE_LOG_CRITICAL("EYETest->EYEParser->ParserExpressionTest->NewExpression Failed to Parse()");
